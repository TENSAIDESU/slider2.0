...
function initEvent(){
    ...
    function initLinks(){
      const links = document.querySelectorAll(".hav");
...
parentDots.innerHTML += `<div class="dot n${index} ${index === 0 ? 'active' : ''}" data-index="${index}"></div>`;
parentLinks.innerHTML += `<div class="hav n${index} ${index === 0 ? 'active' : ''}" data-index="${index}">${images[index].title}</div>`;
      });
    }
    initEvent();
    initLinks(); 
